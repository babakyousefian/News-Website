...
@section('content')
    ...
    @endif
    <form action="{{URL::asset("admin/news/update_check/".$news->id)}}" method="post" enctype="multipart/form-data">
        {{csrf_field()}}
        <div class="form_input_title">عنوان خبر:</div>
        <input type="text" name="title" class="form_input" value="{{$news->title}}">
        @if($errors->has('title'))
...
{{--            <option value="sports">ورزشی</option>--}}
        ...
        <textarea  name="description" class="form_text">{{$news->description}}</textarea>
        @if($errors->has('description'))
            <div class="error_msg">  {{$errors->first('description')}}</div>
        @endif
        <input type="submit" value="ویرایش خبر " class="form_btn">
    </form>
@endsection
